...
to setup-turtles
  ...
  let number-of-immunes ((population-size - number-of-zombies) * (initial-immune-percentage / 100))

  ask turtles [
    ; set turtle on random position
    setxy random-xcor random-ycor

    ifelse (count zombies < number-of-zombies) [
...
to go
  ...
  ask turtles with [breed = zombies] [
    if(any? turtles-here with [breed = humans]) [
      ifelse random 100 < chance-kill-zombie[
        die
      ]
      [
      attack-human
      set energy energy + energy-per-bite
      ]
    ]
  ]
...
"" ""
PENS
"Humans" 1.0 0 -3026479 true "" "plot count humans"
"Zombies" 1.0 0 -2674135 true "" "plot count zombies"
"Immunes" 1.0 0 -13840069 true "" "plot count immunes"
