...
to setup
  ...
  ask patches [
    ...
    set triggered? false
    set last_encountered "defect"
    set score [-99 -99 -99 -99 -99 -99 -99 -99]
    update-color
  ]
...
to go
  ask patches [set played_this_round false]
  ask patches [decide]
...
to decide
  if strategy = "random" [
    ifelse random 100 < 50 [set next_action "cooperate"][set next_action "defect"]
  ]
  if strategy = "always cooperate" [
    set next_action "cooperate"
  ]
  if strategy = "always defect" [
    set next_action "defect"
  ]
  if strategy = "grim trigger" [
    ifelse triggered? [set next_action "defect"][set next_action "cooperate"] ;;sobald man einmal betrogen wurde -> immer defect
  ]
  if strategy = "tit for tat" [
    set next_action last_encountered ;;falls in der letzten Runde kooperiert wurde -> cooperate, sonst defect
  ]
end

to interact  ;; patch procedure
  if not played_this_round[
    let other_player one-of neighbors with [not played_this_round]
    ifelse other_player != nobody[
      play other_player
    ][
      add_score -99
    ]
...
to play [other_patch]
  let other_next_action [next_action] of other_patch
  set last_encountered other_next_action
  ask other_patch [set last_encountered next_action]
  if next_action = "cooperate" and other_next_action = "cooperate" [
    add_score payoff-r
    ask other_patch [add_score payoff-r]
  ]
  if next_action = "cooperate" and other_next_action = "defect" [
    add_score payoff-t
    set triggered? true
    ask other_patch [add_score payoff-s]
  ]
  if next_action = "defect" and other_next_action = "cooperate" [
    add_score payoff-s
    ask other_patch [add_score payoff-t set triggered? true]
  ]
  if next_action = "defect" and other_next_action = "defect" [
    add_score payoff-p
    set triggered? true
    ask other_patch [add_score payoff-p set triggered? true]
  ]
  set played_this_round true
  ask other_patch [set played_this_round true]
end

to add_score [new_score]
  set score fput new_score score
  set score remove-item 8 score
end

to select-strategy  ;; patch procedure
  set old_strategy strategy
  set strategy [strategy] of max-one-of neighbors [sum score]
  if old_strategy = "grimm triggered" and strategy != "grimm triggered" [
    set triggered? false
  ]
end
